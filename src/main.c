/*
 * remoteos-sdl — host-side service for remote draw/audio/input operations
 * over the RemoteOS protocol-v2 byte stream.
 *
 ...
 *   response: {"v":2,"id":<int>,"ok":true,"result":{...}}
 *   error:    {"v":2,"id":<int>,"ok":false,"error":{"code":...,"msg":...}}
 *   event:    {"v":2,"id":0,"op":<str>,"params":{...}}     (host->guest)
 */
...
typedef struct {
    ...
    uint64_t requests, notifications, batches, presents;
    uint64_t dropped_events;
} ServiceTelemetry;

static ServiceTelemetry g_telemetry;

/* ─── Logging ───────────────────────────────────────────────────────────── */

static int g_verbose = 0;
static SDL_AudioDeviceID g_audio_device = 0;
/* When set (by render.batch or an id=0 notification), responses become no-ops.
...
static int g_silent_errors = 0;

static void bridge_log(const char *level, const char *fmt, ...) {
    fprintf(stderr, "[remoteos-sdl:%s] ", level);
    va_list ap;
    va_start(ap, fmt);
...
static int read_frame(int fd, char **out_payload, size_t *out_len) {
    ...
    if (len == 0 || len > REMOTEOS_MAX_FRAME_BYTES) {
        LOG_ERROR("rejecting frame of length %u (cap=%u)", len,
                  REMOTEOS_MAX_FRAME_BYTES);
        return -3;
    }
    ...
    payload[len] = '\0';
    g_telemetry.rx_frames++;
    g_telemetry.rx_bytes += sizeof(be_len) + len;
    *out_payload = payload;
    *out_len = len;
    return 0;
}
...
static int write_frame(int fd, const char *payload, size_t len) {
    if (len > REMOTEOS_MAX_FRAME_BYTES) {
    ...
    g_telemetry.tx_frames++;
    g_telemetry.tx_bytes += sizeof(be_len) + len;
    return 0;
}
...
static cJSON *make_response_envelope(int id) {
    ...
    cJSON_AddNumberToObject(r, "v",  REMOTEOS_PROTOCOL_VERSION);
...
static uint64_t slow_threshold_us(void) {
    const char *value = getenv("REMOTEOS_SDL_SLOW_US");
...
static int op_telemetry_snapshot(BridgeState *st, int id, cJSON *params) {
    int reset = 0;
    ...
    uint64_t now = SDL_GetPerformanceCounter();
    cJSON_AddNumberToObject(result, "uptime_ms", frequency ?
        (double)((now - g_telemetry.started_ticks) * 1000ULL / frequency) : 0);
    cJSON_AddNumberToObject(result, "rx_bytes", (double)g_telemetry.rx_bytes);
    cJSON_AddNumberToObject(result, "tx_bytes", (double)g_telemetry.tx_bytes);
    cJSON_AddNumberToObject(result, "rx_frames", (double)g_telemetry.rx_frames);
    cJSON_AddNumberToObject(result, "tx_frames", (double)g_telemetry.tx_frames);
    cJSON_AddNumberToObject(result, "requests", (double)g_telemetry.requests);
    cJSON_AddNumberToObject(result, "notifications", (double)g_telemetry.notifications);
    cJSON_AddNumberToObject(result, "batches", (double)g_telemetry.batches);
    cJSON_AddNumberToObject(result, "presents", (double)g_telemetry.presents);
    cJSON_AddNumberToObject(result, "dropped_events", (double)g_telemetry.dropped_events);
    cJSON_AddNumberToObject(result, "audio_queued_bytes",
        g_audio_device ? (double)SDL_GetQueuedAudioSize(g_audio_device) : 0);
    ...
    if (reset) {
        ...
        uint64_t started = g_telemetry.started_ticks;
        memset(&g_telemetry, 0, sizeof(g_telemetry));
        g_telemetry.started_ticks = started;
    }
...
static int op_hello(BridgeState *st, int id, cJSON *params) {
    ...
    cJSON *client = cJSON_GetObjectItemCaseSensitive(params, "client");
    if (peer_proto != REMOTEOS_PROTOCOL_VERSION) {
        return send_err(st->fd, id, 100,
                        "protocol mismatch: RemoteOS protocol v2 required");
    }
    if (!cJSON_IsString(client) || !client->valuestring || !client->valuestring[0]) {
        return send_err(st->fd, id, 4, "hello: non-empty client required");
    }
    snprintf(st->client, sizeof(st->client), "%s", client->valuestring);
    st->negotiated = 1;

    cJSON *r = cJSON_CreateObject();
    cJSON_AddNumberToObject(r, "protocol",   REMOTEOS_PROTOCOL_VERSION);
    cJSON_AddStringToObject(r, "service",    "remoteos-sdl");
    cJSON_AddStringToObject(r, "service_ver", REMOTEOS_SERVICE_VERSION);
    cJSON_AddStringToObject(r, "sdl_ver",    SDL_GetRevision());
    cJSON *features = cJSON_CreateArray();
    cJSON_AddItemToArray(features, cJSON_CreateString("render.batch"));
    cJSON_AddItemToArray(features, cJSON_CreateString("frame.commit"));
    ...
    LOG_INFO("negotiated protocol v%d with client=%s",
             REMOTEOS_PROTOCOL_VERSION, st->client);
...
static int op_display_open(BridgeState *st, int id, cJSON *params) {
    ...
    const char *title = cJSON_IsString(jt) ? jt->valuestring : "RemoteOS";
...
static int op_display_present(BridgeState *st, int id, cJSON *params) {
    ...
    g_telemetry.presents++;
    return send_ok(st->fd, id, NULL);
}

static cJSON *take_events(void) {
    drain_sdl_events();
...
static int op_event_poll(BridgeState *st, int id, cJSON *params) {
    (void)params;
    return send_ok(st->fd, id, take_events());
}

/* Present and return input in one round trip. This is the normal animation
 * boundary; guests use id=0 display.present only when they do not need input. */
static int op_frame_commit(BridgeState *st, int id, cJSON *params) {
    (void)params;
    if (!g_window.open) return send_err(st->fd, id, 7, "display not open");
    forget_scaled_frame();
    if (g_window.renderer) {
...
#define FILE_CHUNK_MAX REMOTEOS_FILE_CHUNK_BYTES
...
static const char *export_directory(void) {
    ...
    const char *configured = getenv("REMOTEOS_SDL_EXPORT_DIR");
...
static int op_host_export_begin(BridgeState *st, int id, cJSON *params) {
    ...
    const char *directory = export_directory();
    size_t directory_len = strlen(directory);
    if (directory_len + 64 >= sizeof(slot->temp_path)) {
        slot->token = 0;
        return send_err(st->fd, id, 4, "export directory path too long");
    }
    memcpy(slot->temp_path, directory, directory_len);
    snprintf(slot->temp_path + directory_len,
             sizeof(slot->temp_path) - directory_len,
             "/.remoteos-sdl-export-%d-%d.part", (int)getpid(), token);
...
static int op_host_export_finish(BridgeState *st, int id, cJSON *params) {
    ...
    cJSON *r = cJSON_CreateObject();
    cJSON_AddStringToObject(r, "path", slot->final_path);
    LOG_INFO("exported remote file to %s", slot->final_path);
    slot->token = 0;
    return send_ok(st->fd, id, r);
}
 ...
 * Adding a new function = one entry. A guest need not carry a custom
 ...
 *
 * These are not pure SDL: they answer narrowly scoped questions the guest
 * cannot answer itself, such as host font discovery. Keep this set small so
 * application and language policy remain in the guest.
 */
...
static int wrap_host_default_font_path(BridgeState *st, int id, cJSON *args) {
    (void)args;
...
static const struct {
    ...
    { "host.default_font_path",  wrap_host_default_font_path  },
};
...
static int op_render_batch(BridgeState *st, int id, cJSON *params) {
    cJSON *ops = cJSON_GetObjectItemCaseSensitive(params, "ops");
    if (!cJSON_IsArray(ops)) {
        return send_err(st->fd, id, 4, "render.batch: ops must be array");
    }
    int n = cJSON_GetArraySize(ops);
    if (n > REMOTEOS_MAX_BATCH_OPS) {
        return send_err(st->fd, id, 4, "render.batch: too many operations");
    }
    int errors = 0;
    g_silent_response = 1;
    g_silent_errors = 0;
...
static const struct {
    ...
    { "telemetry.snapshot", op_telemetry_snapshot },
...
static int handle_one_frame(BridgeState *st, const char *payload, size_t len) {
    ...
    if (!cJSON_IsNumber(version) || version->valueint != REMOTEOS_PROTOCOL_VERSION) {
        cJSON_Delete(root);
        return send_err(st->fd, id, 100, "RemoteOS protocol v2 required");
    }
    if (!st->negotiated && strcmp(op->valuestring, "hello") != 0) {
        cJSON_Delete(root);
        return send_err(st->fd, id, 101, "hello must be the first request");
    }
    if (st->negotiated && strcmp(op->valuestring, "hello") == 0) {
        cJSON_Delete(root);
        return send_err(st->fd, id, 102, "session already negotiated");
    }
    if (id == 0) g_telemetry.notifications++;
    else g_telemetry.requests++;
...
static int serve_fd(int fd) {
    BridgeState st = { .fd = fd, .should_exit = 0, .negotiated = 0 };
    memset(&g_telemetry, 0, sizeof(g_telemetry));
    g_telemetry.started_ticks = SDL_GetPerformanceCounter();
...
static int socket_buffer_bytes(void) {
    const char *value = getenv("REMOTEOS_SDL_SOCKET_BUFFER");
    if (!value || !value[0]) return 4 * 1024 * 1024;
    char *end = NULL;
    long parsed = strtol(value, &end, 10);
    if (end == value || *end != '\0' || parsed < 65536 ||
        parsed > 64 * 1024 * 1024) {
        LOG_WARN("ignoring invalid REMOTEOS_SDL_SOCKET_BUFFER=%s", value);
        return 4 * 1024 * 1024;
    }
    return (int)parsed;
}
...
static int run_selftest(void) {
    ...
    SDL_Window *w = SDL_CreateWindow("remoteos-sdl selftest",
                                     SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
...
static void usage(FILE *out, const char *argv0) {
    fprintf(out,
        ...
        "  --connect-tcp HOST:PORT connect to a guest TCP bridge listener\n"
